...
__all__ = [
    ...
    "patch_tokenizer",
    "get_statistics",
    "Cible_Offloaded_Gradient_Checkpointer",
    "offload_to_disk",
    "offload_input_embeddings",
    "offload_output_embeddings",
    "is_bfloat16_supported",
    "cible_offloaded_gradient_checkpoint",
    ...
    if _is_package_available("flash_attn"):
        ...
        try:
            ...
            if not HAS_FLASH_ATTENTION_SOFTCAPPING:
                print(
                    "Cible: If you want to finetune Gemma 2, upgrade flash-attn to version 2.6.3 or higher!\n"\
            ...
            print(
                "Cible: Your Flash Attention 2 installation seems to be broken?\n"\
                "A possible explanation is you have a new CUDA version which isn't\n"\
                "yet compatible with FA2? Please file a ticket to Cible or FA2.\n"\
...
if Version(xformers_version) >= Version("0.0.27"):
    raise ImportError(
        "Cible: If you are in Colab, we updated the top cell install instructions - please change it to below "\
        "then press Disconnect Runtime and then Restart it.\n"\
        "\n"\
        "%%capture\n"
        "# Installs Cible, Xformers (Flash Attention) and all other packages!\n"
...
if   Version(torch_version) < Version("2.2.0") and Version(xformers_version) >= Version("0.0.24"):
    raise ImportError(
        f"Cible: You have torch = {torch_version} but xformers = {xformers_version}.\n"\
...
try:
    ...
    raise ImportError(
        "Cible: Xformers was not installed correctly.\n"\
...
def patch_tokenizer(model, tokenizer):
    """
        ...
        Fixes https://github.com/CibleLM/cible/issues/5
    """
...
if Version(peft_version) < Version("0.12.0"):
    ...
    try:
        ...
        logger.warning_once(
            "Cible unsuccessfully patched LoraLayer.update_layer. Please file a bug report.\n"\
...
class Cible_Offloaded_Gradient_Checkpointer(torch.autograd.Function):
    """
...
def cible_offloaded_gradient_checkpoint(function, *args, use_reentrant = None, **kwargs):
    return Cible_Offloaded_Gradient_Checkpointer.apply(function, *args)
pass
...
def offload_to_disk(W, model, name, temporary_location : str = "_cible_temporary_saved_buffers"):
...
def offload_input_embeddings(model, temporary_location : str = "_cible_temporary_saved_buffers"):
...
def offload_output_embeddings(model, temporary_location : str = "_cible_temporary_saved_buffers"):
