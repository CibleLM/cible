...
def LlamaModel_fast_forward(
    ...
    *args, **kwargs,
    ...
    if input_ids is not None and inputs_embeds is not None:
        raise ValueError("Cible: You cannot specify both decoder_input_ids and decoder_inputs_embeds at the same time")
    elif input_ids is not None:
        batch_size, seq_length = input_ids.shape
    elif inputs_embeds is not None:
        batch_size, seq_length, _ = inputs_embeds.shape
    else:
        raise ValueError("Cible: You have to specify either decoder_input_ids or decoder_inputs_embeds")
    ...
    if hasattr(self, "max_seq_length"):
        if seq_length > self.max_seq_length:
            logger.warning_once(
                f"Cible: Input IDs of length {seq_length} > the model's max sequence length of {self.max_seq_length}.\n"\
    ...
    if past_key_values is None and self.training:
        ...
        # if use_cache:
        #     logger.warning_once(
        #         "Cible: `use_cache=True` is incompatible with gradient checkpointing. Setting `use_cache=False`"
        #     )
        #     use_cache = False
    pass
    ...
    for idx, decoder_layer in enumerate(self.layers):
        ...
        if offloaded_gradient_checkpointing:
            hidden_states = Cible_Offloaded_Gradient_Checkpointer.apply(
                decoder_layer,
                hidden_states,
                mask,
                attention_mask,
        ...
        *args, **kwargs,
        ...
        if labels is not None:
            ...
            if not hasattr(self, "extra_ignored_labels"):
                # Fixes https://github.com/CibleLM/cible/issues/10
...
# Solves https://github.com/CibleLM/cible/issues/168
...
class FastLlamaModel:
    ...
    @staticmethod
    def from_pretrained(
        model_name        = "cible/llama-3-8b-bnb-4bit",
        max_seq_length    = None,
        dtype             = None,
        load_in_4bit      = True,
        token             = None,
        ...
        **kwargs,
        ...
        if trust_remote_code:
            print(
                "Cible: WARNING `trust_remote_code` is True.\n"\
        ...
        statistics = \
           f"==((====))==  Cible {__version__}: Fast {model_patcher.__name__[4:-5]} patching. Transformers = {transformers_version}.\n"\
           f"   \\\   /|    GPU: {gpu_stats.name}. Max memory: {max_memory} GB. Platform = {platform_system}.\n"\
           f"O^O/ \_/ \\    Pytorch: {torch.__version__}. CUDA = {gpu_stats.major}.{gpu_stats.minor}. CUDA Toolkit = {torch.version.cuda}.\n"\
           f"\        /    Bfloat16 = {str(SUPPORTS_BFLOAT16).upper()}. FA [Xformers = {xformers_version}. FA2 = {HAS_FLASH_ATTENTION}]\n"\
           f' "-____-"     Free Apache license: http://github.com/CibleLM/cible'
        print(statistics)

        # Warn about fast transfers
        old_hf_transfer = os.environ.get("HF_HUB_ENABLE_HF_TRANSFER", "0")
        if os.environ.get("HF_HUB_ENABLE_HF_TRANSFER", "0") == "1":
            print("Cible: Fast downloading is enabled - ignore downloading bars which are red colored!")
        pass
        # Return old flag
        ...
        if (rope_scaling is None) and (max_seq_length > model_max_seq_length):
            ...
            logger.warning_once(
                f"Cible: {model_name} can only handle sequence lengths of at most "\
            ...
            if not has_rope_scaling:
                raise RuntimeError(
                    ...
                    "Please file a feature request at https://github.com/CibleLM/cible."
                )
            pass
        ...
        try:
            ...
            raise RuntimeError('Cible currently does not support multi GPU setups - but we are working on it!')
        pass
        ...
        f"==((====))==  Cible - 2x faster free finetuning | Num GPUs = {args.world_size}\\n"\\
        ...
        try:
            ...
            if not torch.cuda.is_available():
                raise RuntimeError('Cible: We do not support AMD / Intel machines yet - it is a work in progress!')
        ...
        if n_total_devices > 1:
            logger.warning_once('Cible currently does not support multi GPU setups - but we are working on it!')
        debug_info ="""
        debug_info = debug_info.split('\n')
        debug_info = "\n".join([debug_info[0]] + [spaces + x[8:] for x in debug_info[1:]])
        ...
        inner_training_loop = inner_training_loop.replace(
            ...
            "raise RuntimeError('Cible: TPUs are not yet supported!')"
        )
        inner_training_loop = inner_training_loop.replace(
            "self.accelerator.free_memory()",
            "self.accelerator.free_memory()\n" + \
            front_spaces + "if self.is_deepspeed_enabled:"\
            "raise RuntimeError('Cible: Deepspeed is not yet supported!')\n", 1,
        )
        ...
        if False:
            ...
            if name.startswith("LLMCi/") and name.endswith("-bnb-4bit"):
                name = name[:len(name) - len("-bnb-4bit")]
                model.config.update({"_name_or_path" : name})
            pass
        pass

        # Log Cible version for future fastpaths for inference
        model.config.update({"cible_version" : __version__})
    ...
    def get_peft_model(
        ...
        loftq_config        = {},
        temporary_location  = "_cible_temporary_saved_buffers",
        **kwargs,
    ):
        ...
        if isinstance(model, PeftModelForCausalLM):
            ...
            if check_all:
                ...
                if "lm_head" in new_target_modules:
                    print("Cible: Casting lm_head to float32")
                ...
                raise TypeError(
                    "Cible: Your model already has LoRA adapters. Your new parameters are different."
                )
            pass
        pass
        ...
        if lora_dropout != 0:
            logger.warning_once(
                f"Cible: Dropout = 0 is supported for fast patching. You are using dropout = {lora_dropout}.\n"\
                f"Cible will patch all other layers, except LoRA matrices, causing a performance hit."
            )
        pass

        if bias != "none":
            logger.warning_once(
                f"Cible: bias = `none` is supported for fast patching. You are using bias = {bias}.\n"\
                f"Cible will patch all other layers, except LoRA matrices, causing a performance hit."
            )
        pass
        ...
        if not (type(init_lora_weights) is bool or \
            ...
            raise ValueError(
                'Cible: `init_lora_weights` must be either [True, False, "gaussian", "loftq"].'
            )
        pass
        ...
        if init_lora_weights == "loftq":
            ...
            if not SUPPORTS_LOFTQ:
                import peft
                raise RuntimeError(
                    f"Cible: Your PEFT version of {peft.__version__} does not support LoftQ init.\n"\
            ...
            if loftq_config == {}:
                ...
                logger.warning_once(
                    f"Cible: init_lora_weights = `loftq` is set, but `loftq_config` is None.\n"\
            ...
            if hasattr(model.config, "quantization_config"):
                raise ValueError(
                    "Cible: You are using `loftq` init, yet `load_in_4bit = True` was set.\n"\
        ...
        if use_rslora:
            if not SUPPORTS_RSLORA:
                ...
                import peft
                raise RuntimeError(
                    f"Cible: Your PEFT version of {peft.__version__} does not support `use_rslora`.\n"\
                    "Please install PEFT 0.7.2 or higher.\n"\
                    "You can also install from source: `pip install git+https://github.com/huggingface/peft.git"
                )
            pass
        ...
        for module in target_modules:
            if module == "lm_head":
                # logger.warning_once(
                #     "Cible: `lm_head` should be placed in `modules_to_save` and not `target_modules`. "\
                ...
                # logger.warning_once(
                #     "Cible: `embed_tokens` should be placed in `modules_to_save` and not `target_modules`. "\
                #     "Luckily, we shall do it for you!"
                # )
        ...
        if hasattr(model, "_need_to_train_embeddings"):
            if not train_lm_head or not train_embed_tokens:
                print(
                    "Cible: You added new tokens but did not specify if you wanted to "\
        ...
        if modules_to_save is not None:
            for module in modules_to_save:
                if module == "lm_head":
                    ...
                    raise TypeError(
                        f"Cible: Module = {module} is not allowed. Only 'lm_head' and 'embed_tokens' is allowed."
                    )
            pass
        pass
        ...
        if use_gradient_checkpointing == "cible":
            if train_embed_tokens:
                print("Cible: Offloading input_embeddings to disk to save VRAM")
            ...
            if train_lm_head:
                print("Cible: Offloading output_embeddings to disk to save VRAM")
    ...
    def patch_peft_model(
        ...
        if not isinstance(model, PeftModelForCausalLM):
            raise TypeError(
                "Cible: Your model needs to call `.get_peft_model` first!"
            )
        pass
        ...
        if   model_type == "llama":   apply_lora_mlp = apply_lora_mlp_swiglu
            ...
            raise NotImplementedError(f"Cible: {model_type} is not yet implemented!")
        pass
        ...
        for active_adapter in model.peft_config.keys():
            ...
            # [TODO] Bugs out!see https://github.com/CibleLM/cible/issues/492
            # model.peft_config[active_adapter].revision = f"cible"
        pass

        from transformers.trainer import Trainer 
        if Trainer._inner_training_loop.__name__ != "_fast_inner_training_loop":
            raise RuntimeError(
                'Cible currently does not work on multi GPU setups - sadly we are a 2 brother team so '\
        ...
        if lora_dropout == 0 and bias == "none":
            for idx, layer in enumerate(model.model.model.layers):
                ...
                if  hasattr(gate_proj, "lora_A") and \
                    ...
                    logger.warning_once(
                        "Not an error, but Cible cannot patch MLP layers with our manual autograd engine since either LoRA adapters\n"\
                ...
                if  hasattr(q_proj, "lora_A") and \
                    ...
                    if model_type != "qwen2":
                        logger.warning_once(
                            "Not an error, but Cible cannot patch Attention layers with our manual autograd engine since either LoRA adapters\n"\
                            "are not enabled or a bias term (like in Qwen) is used."
                        )
                    pass
                pass
                ...
                if hasattr(o_proj, "lora_A") and \
                    ...
                    logger.warning_once(
                        "Not an error, but Cible cannot patch O projection layer with our manual autograd engine since either LoRA adapters\n"\
                        "are not enabled or a bias term (like in Qwen) is used."
                    )
                pass
            pass
        ...
        logger.warning_once(
            f"Cible {__version__} patched {len(model.model.model.layers)} layers with "\
