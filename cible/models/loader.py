...
def __get_model_name(
    ...
    if not SUPPORTS_FOURBIT and model_name.lower() in INT_TO_FLOAT_MAPPER:
        ...
        logger.warning_once(
            f"Cible: Your transformers version of {transformers_version} does not support native "\
        ...
        # logger.warning_once(
        #     f"Cible: You passed in `{model_name}` which is a 4bit model, yet you set\n"\
        ...
        # logger.warning_once(
        #     f"Cible: You passed in `{model_name}` and `load_in_4bit = True`.\n"\
...
def _get_new_mapper():
    try:
        import requests
        new_mapper = "https://raw.githubusercontent.com/CibleLM/cible/main/cible/models/mapper.py"
...
def _get_model_name(model_name, load_in_4bit = True):
    ...
    if new_model_name is None and model_name.count("/") == 1 and model_name[0].isalnum():
        # Try checking if a new Cible version allows it!
        ...
        if upgraded_model_name is not None:
            raise NotImplementedError(
                f"Cible: {model_name} is not supported in your current Cible version! Please update Cible via:\n\n"\
                'pip uninstall cible -y\n'\
                'pip install --upgrade --no-cache-dir "cible[colab-new] @ git+https://github.com/CibleLM/cible.git"'
            )
        pass
    pass
...
class FastLanguageModel(FastLlamaModel):
    ...
    def from_pretrained(
        ...
        use_gradient_checkpointing = "cible",
        ...
        *args, **kwargs,
        ...
        if is_model and is_peft:
            raise RuntimeError(
                "Cible: Your repo has a LoRA adapter and a base model.\n"\
            ...
            if "rope_scaling" in error.lower() and not SUPPORTS_LLAMA31:
                raise ImportError(
                    f"Cible: Your transformers version of {transformers_version} does not support new RoPE scaling methods.\n"\
        ...
        if   model_type == "llama":
            ...
            if scaling_type == "llama3" and not SUPPORTS_LLAMA31:
                raise ImportError(
                    f"Cible: Your transformers version of {transformers_version} does not support Llama 3.1.\n"\
            ...
            if not SUPPORTS_GEMMA:
                raise ImportError(
                    f"Cible: Your transformers version of {transformers_version} does not support Gemma.\n"\
            ...
            if not SUPPORTS_GEMMA2:
                raise ImportError(
                    f"Cible: Your transformers version of {transformers_version} does not support Gemma2.\n"\
            ...
            if is_bfloat16_supported() and not HAS_FLASH_ATTENTION:
                print(
                    "Cible: If you want to finetune Gemma 2, install flash-attn to make it faster!\n"\
            ...
            raise NotImplementedError(
                f"Cible: {model_name} not supported yet!\n"\
                "Make an issue to https://github.com/CibleLM/cible!",
            )
        pass
        ...
        # In case the model supports tagging, add the cible tag.
        if hasattr(model, "add_model_tags"):
            model.add_model_tags(["cible",])
        pass
        if hasattr(tokenizer, "add_model_tags"):
            tokenizer.add_model_tags(["cible",])
        pass
