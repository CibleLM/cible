...
# =========================================== Cible
# Cible efficient template leverages from Zephyr
cible_template = \
    "{{ bos_token }}"\
...
cible_eos_token = "eos_token"
CHAT_TEMPLATES["cible"] = (cible_template, cible_eos_token, False, cible_ollama,)
pass
...
def get_chat_template(
    ...
    if type(chat_template) in (list, tuple,):
        ...
        if not is_fast_tokenizer:
            print(
                f"Cible: Not a fast tokenizer, so can't process it as of yet :(\n"\
            ...
            if map_eos_token and (not stop_word in token_mapping.values()):
                ...
                logger.warning_once(f"Cible: Will map {stop_word} to EOS = {tokenizer.eos_token}.")
        ...
        raise TypeError(
            f"Cible: `chat_template` must be a tuple of (your_template, eos_token,) or one of\n"\
...
def _parse_combined_prompt(combined_prompt, dataset):
    ...
    for column in possible_columns:
        if column not in dataset_columns:
            raise KeyError(
                f"Cible: Your prompt includes '{column}' but this does not exist in the dataset. "\
...
def to_sharegpt(
    ...
    if "conversations" in dataset.column_names:
        ...
        if type(convo) is list:
            raise TypeError("Cible: Your dataset is probably already in ShareGPT format!")
        pass
    pass
...
def standardize_sharegpt(
    ...
    if len(leftover_aliases) != 0:
        raise TypeError(
            f"Cible: {list(leftover_aliases)} are not in aliases. Please update aliases."
        )
    pass
...
extra_eos_tokens = None,
    ...
    for extra_eos in extra_eos_tokens:
        ...
        if extra_eos not in vocab:
            raise ValueError(f"Cible: `{extra_eos}` is not a singular token in the tokenizer.")
        pass
    pass

    error_msg = \
        "Cible: Your prompt template must have 2 examples showing the user input {INPUT} "\
    ...
    if len(extra_eos_tokens) == 0:
        raise RuntimeError(
            "Cible: Your tokenizer does not have an EOS token? Please provide one via extra_eos_tokens!"
        )
    pass

    # Check tokenizer types
    tokenizer_name = tokenizer.name_or_path.lower()
    if tokenizer_name.startswith(("cible/llama-3-8b-instruct", "cible/llama-3-70b-instruct")):
        # Add <|eot_id|>
        extra_eos_tokens.append("<|eot_id|>")
    elif ("<|eot_id|>" in extra_eos_tokens or "<|eot_id|>" in chat_template) and \
        tokenizer_name.startswith(("cible/llama-3-8b", "cible/llama-3-70b")):
        # Warn
        logger.warning(
            "Cible: Base llama-3 models did not train <|eot_id|>.\n"\
    ...
    try:
        ...
        if combined != final_combined_check:
            ...
            raise RuntimeError(
                "Cible: The prompt template you provided isn't correct. You gave:\n"\
                f"{combined_changed}\n\n"\
    ...
    if count_eos == 0:
        logger.warning("Cible: We automatically added an EOS token to stop endless generations.")
    ...
    if len(system_part) != 0:
        ...
        if "{SYSTEM}" in partial_system:
            if default_system_message is None:
                raise RuntimeError("Cible: Please specify a default system message!")
        pass
...
extra_eos_tokens = None,
    ...
    tokenizer._cible_input_part  = input_part
    tokenizer._cible_output_part = output_part
...
def train_on_responses_only(
    ...
    if  not hasattr(tokenizer, "_cible_input_part") or \
        not hasattr(tokenizer, "_cible_output_part"):
        
        if instruction_part is None or response_part is None:
            raise ValueError("Cible: instruction_part and response_part must be given!")
        pass
    elif (instruction_part is not None or response_part is not None) and \
        (hasattr(tokenizer, "_cible_input_part") or hasattr(tokenizer, "_cible_output_part")):

        raise ValueError("Cible: Your tokenizer already has instruction and response parts set - do not give custom ones!")
    else:
        instruction_part = tokenizer._cible_input_part
        response_part    = tokenizer._cible_output_part
    pass
...
def test_chat_templates():
    ...
    correct_tokenizer = AutoTokenizer.from_pretrained("cible/llama-2-7b-chat")
    correct_prompt = correct_tokenizer.apply_chat_template(messages, tokenize = False, add_generation_prompt = True)
    correct_tokenizer.chat_template = template
    our_prompt = correct_tokenizer.apply_chat_template(messages, tokenize = False, add_generation_prompt = True)
    assert(correct_prompt == our_prompt)
    ...
    # Gemma
    correct_tokenizer = AutoTokenizer.from_pretrained("cible/gemma-7b-it")
    correct_prompt = correct_tokenizer.apply_chat_template(messages[1:], tokenize = False, add_generation_prompt = True)
    ...
    correct_tokenizer = AutoTokenizer.from_pretrained("cible/llama-3-8b-Instruct")
    correct_prompt = correct_tokenizer.apply_chat_template(messages, tokenize = False, add_generation_prompt = True)
    correct_tokenizer.chat_template = template
    our_prompt = correct_tokenizer.apply_chat_template(messages, tokenize = False, add_generation_prompt = True)
    assert(correct_prompt == our_prompt)
...
def test_hf_gguf_equivalence(tokenizer, gguf_model = "./model-cible.F16.gguf"):
    """
