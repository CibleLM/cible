...
#         raise ImportError(f"Cible: Please import Cible before {module}.")
#     pass
# pass
...
if "CUDA_VISIBLE_DEVICES" in os.environ:
    ...
    if not devices.isdigit():
        ...
        warnings.warn(
            f"Cible: 'CUDA_VISIBLE_DEVICES' is currently {devices} \n"\
            "Cible currently does not support multi GPU setups - but we are working on it!\n"\
            "Multiple CUDA devices detected but we require a single device.\n"\
            f"We will override CUDA_VISIBLE_DEVICES to first device: {first_id}."
        )
        os.environ["CUDA_VISIBLE_DEVICES"] = str(first_id)
else:
    # warnings.warn("Cible: 'CUDA_VISIBLE_DEVICES' is not set. We shall set it ourselves.")
    os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
...
# Fixes https://github.com/CibleLM/cible/issues/38
torch_version = torch.__version__.split(".")
major_torch, minor_torch = torch_version[0], torch_version[1]
major_torch, minor_torch = int(major_torch), int(minor_torch)
if (major_torch < 2):
    raise ImportError("Cible only supports Pytorch 2 for now. Please update your Pytorch to 2.1.\n"\
                      "We have some installation instructions on our Github page.")
...
try:
    ...
    warnings.warn(
        "Cible: Running `ldconfig /usr/lib64-nvidia` to link CUDA."\
    )
    ...
    try:
        ...
        warnings.warn(
            "Cible: CUDA is not linked properly.\n"\
            "Try running `python -m bitsandbytes` then `python -m xformers.info`\n"\
            "We tried running `ldconfig /usr/lib64-nvidia` ourselves, but it didn't work.\n"\
            "You need to run in your terminal `sudo ldconfig /usr/lib64-nvidia` yourself, then import Cible.\n"\
            "Also try `sudo ldconfig /usr/local/cuda-xx.x` - find the latest cuda version.\n"\
            "Cible will still run for now, but maybe it might crash - let's hope it works!"
        )
pass
