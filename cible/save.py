...
def check_if_sentencepiece_model(model, temporary_location = "_cible_sentencepiece_temp"):
...
def _free_cached_model(model):
    ...
    for cached_repo in cached_repos:
        if cached_repo.repo_id == model.config._name_or_path:
            ...
            logger.warning_once(
                "Cible: Will remove a cached repo with size " + \
    ...
    if isinstance(layer, (Bnb_Linear4bit, Peft_Linear4bit, Peft_Linear)):
        ...
        if A is not None:
            ...
            if not torch.isfinite(maximum_element).item():
                raise ValueError(f"Cible: Merge failed.\n{name} has some elements = infinity.")
        pass
        W = W.t().to(dtype)
    else:
        W = layer.weight
...
def cible_save_model(
    ...
    commit_message       : Optional[str] = "Trained with Cible",
    private              : Optional[bool] = None,
    create_pr            : bool = False,
    revision             : str = None,
    commit_description   : str = "Télécharger des fichiers en utilisant Cible",
    tags                 : List[str] = None,

    # Our functions
    temporary_location   : str = "_cible_temporary_saved_buffers",
    ...
    if "Cible" not in commit_message:
        commit_message += " (Trained with Cible)"
    commit_message = commit_message.lstrip()

    if commit_description is None:
        commit_description = "Télécharger des fichiers en utilisant Cible"
    elif "Cible 2x faster" not in commit_description:
        commit_description += " (Trained with Cible 2x faster)"
    pass

    if save_method == "merged_4bit":
        raise RuntimeError(
            "Cible: Merging into 4bit will cause your model to lose accuracy if you plan\n"\
    ...
    if save_method != "lora" and save_method != "merged_16bit" and save_method != "merged_4bit":
        raise RuntimeError(
            "Cible: You must select one of 3 options when saving models:\n"\
    ...
    if save_method == "merged_4bit":
        ...
        print("Cible: Merging 4bit and LoRA weights to 4bit...")
    ...
    if ((save_method == "lora") or (save_method == "merged_4bit")) and push_to_hub:
        if token is None:
            raise RuntimeError(
                "Cible: Pushing to HF requires a token. Pass `token = 'hf_....'`\n"\
                "Go to https://huggingface.co/settings/tokens."
            )
        pass

        if save_method == "lora":
            print("Cible: Saving LoRA adapters. Please wait...")
        elif save_method == "merged_4bit":
            print("Cible: Saving 4bit Bitsandbytes model. Please wait...")
        pass
    ...
    if (save_method == "merged_4bit") or (save_method == "lora") or (
        ...
        if tokenizer is not None:
            print("Cible: Saving tokenizer...", end = "")
    ...
    if push_to_hub and "/" in save_directory:
        ...
        logger.warning_once(
            f"Cible: You are pushing to hub, but you passed your HF username = {username}.\n"\
    ...
    print("Cible: Merging 4bit and LoRA weights to 16bit...")
    ...
    if safe_serialization is None:
        ...
        logger.warning_once(
            f"Cible: You have {n_cpus} CPUs. Using `safe_serialization` is 10x slower.\n"\
    ...
    if IS_KAGGLE_ENVIRONMENT or IS_COLAB_ENVIRONMENT:
        ...
        logger.warning_once(
            "Cible: Kaggle/Colab has limited disk space. We need to delete the downloaded\n"\
    ...
    for key, value in state_dict.items():
        ...
        if type(value) is not torch.Tensor:
            logger.warning_once(f"Cible: {key} is not a Tensor but a {type(value)}.")
        pass
    pass
    ...
    if save_pretrained_settings["push_to_hub"] and (username != actual_username):
        print(f"Cible: Saving to organization with address {new_save_directory}")
    ...
    print("Cible: Saving model... This might take 5 minutes for Llama-7b...")
    ...
    if save_pretrained_settings["push_to_hub"] and (username != actual_username):
        ...
        print("Cible: Uploading all files... Please wait...")
...
def install_llama_cpp_old(version = -10):
    ...
    if os.path.exists("llama.cpp"):
        print(
            ...
            "Cible will DELETE the broken directory and install a new one.\n"\
    ...
    if not os.path.exists("llama.cpp/quantize") and not os.path.exists("llama.cpp/llama-quantize"):
        raise RuntimeError(
            "Cible: The file 'llama.cpp/llama-quantize' or `llama.cpp/quantize` does not exist.\n"\
...
def save_to_gguf(
    ...
    model_directory      : str = "cible_finetuned_model",
    ...
    #     "undergoing some major bug fixes as at 5th of May 2024. This is not an Cible issue.\n"\
    ...
    if   isinstance(quantization_method, list):  pass
        ...
        raise TypeError("Cible: quantization_method can only be a string or a list of strings")
    pass
    
    # Check if bfloat16 is supported
    if model_dtype == "bf16" and not torch.cuda.is_bf16_supported():
        logger.warning(
            "Cible: Cannot convert to bf16 GGUF since your computer doesn't support it.\n"\
    ...
    for quant_method in quantization_method: 
        if quant_method.startswith("iq2"):
            raise RuntimeError("Cible: Currently iq2 type quantizations aren't supported yet - sorry!")
    pass
    ...
    pass
    logger.warning_once(f"Cible: Converting {model_type} model. Can use fast conversion = {use_fast_convert}.")
    ...
    for quant_method in quantization_method:
        ...
        if quant_method not in ALLOWED_QUANTS.keys():
            error = f"Cible: Quant method = [{quant_method}] not supported. Choose from below:\n"
            for key, value in ALLOWED_QUANTS.items():
                error += f"[{key}] => {value}\n"
    ...
    print_info = \
        f"==((====))==  Cible: Conversion from QLoRA to GGUF information\n"\
    ...
    if   first_conversion == "f16"  : pass
        ...
        raise RuntimeError(
            f"Cible: `first_conversion` can only be one of ['f16', 'bf16', 'f32', 'q8_0'] and not `{first_conversion}`."
        )
    pass
    ...
    if quantize_location is not None and convert_location is not None:
        print("Cible: llama.cpp found in the system. We shall skip installation.")
    else:
        print("Cible: [0] Installing llama.cpp. This will take 3 minutes...")
        ...
        # See https://github.com/CibleLM/cible/pull/730
        ...
        if os.path.exists("llama.cpp/convert-hf-to-gguf.py"):
            ...
            raise RuntimeError(
                "Cible: The file 'llama.cpp/convert-hf-to-gguf.py' or 'llama.cpp/convert_hf_to_gguf.py' does not exist.\n"\
                "But we expect this file to exist! Maybe the llama.cpp developers changed the name?"
            )
        pass

        if error != 0 or quantize_location is None or convert_location is None:
            print(f"Cible: llama.cpp error code = {error}.")
    ...
    for quant_method in quantization_method:
        if   quant_method == "f32":  strength = max(strength, 3)
            ...
            if   first_conversion == "f32"  : pass
                ...
                logger.warning_once(
                    "Cible: Using q8_0 for the `first_conversion` will lose a bit of accuracy, "\
    ...
    if not use_fast_convert and \
        ...
        # logger.warning_once("Cible: We must use f16 for non Llama and Mistral models.")
        # first_conversion = "f16"
    pass
    ...
    final_location = f"./{model_directory}/cible.{first_conversion.upper()}.gguf"

    print(f"Cible: [1] Converting model at {model_directory} into {first_conversion} GGUF format.\n"\
    ...
    if not os.path.isfile(final_location):
        if IS_KAGGLE_ENVIRONMENT:
            raise RuntimeError(
                f"Cible: Quantization failed for {final_location}\n"\
        ...
        pass
    pass
    print(f"Cible: Conversion completed! Output location: {final_location}")
    ...
    for quant_method in quantization_method:
        if quant_method != first_conversion:
            print(f"Cible: [2] Converting GGUF 16bit into {quant_method}. This will take 20 minutes...")
            final_location = f"./{model_directory}/cible.{quant_method.upper()}.gguf"
            ...
            if not os.path.isfile(final_location):
                if IS_KAGGLE_ENVIRONMENT:
                    ...
                    raise RuntimeError(
                        "Cible: Quantization failed! You might have to compile llama.cpp yourself, then run this again.\n"\
                        "You do not need to close this Python program. Run the following commands in a new terminal:\n"\
                        "You must run this in the same folder as you're saving your model.\n"\
                        "git clone --recursive https://github.com/ggerganov/llama.cpp\n"\
                        "cd llama.cpp && make clean && make all -j\n"\
...
def cible_save_pretrained_merged(
    self,
    save_directory       : Union[str, os.PathLike],
    tokenizer            = None,
    ...
    if tokenizer is None:
        logger.warning_once(
            "Cible: You're not saving a tokenizer as well?\n"\
    ...
    del arguments["self"]
    cible_save_model(**arguments)
    for _ in range(3):
        gc.collect()
pass


def cible_push_to_hub_merged(
    self,
    repo_id              : str,
    tokenizer            = None,
    save_method          : str = "merged_16bit", # ["lora", "merged_16bit", "merged_4bit"]
...
This {model_type} model was trained 2x faster with [Cible](https://github.com/CibleLM/cible) and Huggingface's TRL library.

[<img src="https://raw.githubusercontent.com/CibleLM/cible/main/images/cible%20made%20with%20love.png" width="200"/>](https://github.com/CibleLM/cible)
"""
...
def _determine_username(save_directory, old_username, token):
    ...
    if "/" not in save_directory:
        ...
        try: 
            ...
            raise RuntimeError(f"Cible: {save_directory} is not a Huggingface directory.")
    else:
...
def upload_to_huggingface(
    ...
    if file_location is not None:
        ...
        if len(ftevent_files) > 0:
            print("Cible: Uploading tensorboard files... Please wait...", file_location + "*out.tfevents*")
        ...
        import json
        with open("_temporary_cible_config.json", "w") as file:
            json.dump({"model_type" : model.config.model_type}, file, indent = 4)
        pass
        hf_api.upload_file(
            path_or_fileobj = "_temporary_cible_config.json",
            path_in_repo    = "config.json",
            repo_id         = save_directory,
            repo_type       = "model",
            commit_message  = "(Trained with Cible)",
        )
        os.remove("_temporary_cible_config.json")
    pass
    return username
pass
...
def fix_tokenizer_bos_token(tokenizer):
    ...
    if (tokenizer("A").input_ids[0] == getattr(tokenizer, "bos_token_id", None)):
        if chat_template is not None and \
            ...
            fix_bos_token = True
            logger.warning(
                f"Cible: ##### The current model auto adds a BOS token.\n"\
                "Cible: ##### Your chat template has a BOS token. We shall remove it temporarily."
            )
...
def cible_save_pretrained_gguf(
    self,
    save_directory       : Union[str, os.PathLike],
    tokenizer            = None,
    ...
    """
    if tokenizer is None:
        raise ValueError("Cible: Saving to GGUF must have a tokenizer.")
    ...
    if not os.path.exists("llama.cpp"):
        ...
        if IS_KAGGLE_ENVIRONMENT:
            ...
            new_save_directory, old_username = cible_save_model(**arguments)
            makefile = None
        else:
    ...
    if type(model_dtype) is str:
        ...
        raise TypeError("Cible: Model dtype can only be float16 or bfloat16")
    pass
    ...
    if modelfile is not None:
        ...
        pass
        print(f"Cible: Saved Ollama Modelfile to {modelfile_location}")
    pass

    if fix_bos_token:
        logger.warning(
            f"Cible: ##### The current model auto adds a BOS token.\n"\
            "Cible: ##### We removed it in GGUF's chat template for you."
        )
    pass

    if push_to_hub:
        print("Cible: Uploading GGUF to Huggingface Hub...")
...
def save_lora_to_custom_dir(model, tokenizer, save_directory):
    ...
    # Call the cible_save_model function with the custom directory
    cible_save_model(
        model,
        tokenizer,
...
def cible_convert_lora_to_ggml_and_push_to_hub(
    self,
    tokenizer,
    repo_id: str,
    use_temp_dir: Optional[bool] = None,
    commit_message: Optional[str] = "Converted LoRA to GGML with Cible",
    private: Optional[bool] = None,
    token: Union[bool, str, None] = None,
    create_pr: bool = False,
    revision: str = None,
    commit_description: str = "Convert LoRA to GGML format using Cible",
    temporary_location: str = "_cible_temporary_saved_buffers",
    maximum_memory_usage: float = 0.85,
):
    if not os.path.exists("llama.cpp"):
    ...
    print(f"Cible: Converting auto-saved LoRA adapters at {lora_directory_push} to GGML format.")
    ...
    print(f"Cible: Conversion completed! Output file: {output_file}")

    print("Cible: Uploading GGML file to Hugging Face Hub...")
    username = upload_to_huggingface(
        self, repo_id, token,
    ...
    print("\nThis GGML making function was made by Maheswar. Ping him @Maheswar on the Cible Discord or on HuggingFace (@mahiatlinux) if you like this!")

def cible_convert_lora_to_ggml_and_save_locally(
    self,
    ...
    print(f"Cible: Converting auto-saved LoRA adapters at {save_directory} to GGML format.")
    print(f"The output file will be {output_file}")
...
def patch_saving_functions(model):
    ...
    if model.push_to_hub.__name__ == "cible_push_to_hub":
    ...
    push_to_hub_text = f'''def cible_push_to_hub(self, {signature}:
    """
    {docs}
    """
    arguments = dict(locals())
    ...
    if "tags" in arguments and arguments["tags"] is not None:
        ...
        arguments["tags"] = list(arguments["tags"]) + ["cible",]
    elif "tags" in arguments:
        arguments["tags"] = ["cible",]
    elif hasattr(self, "add_model_tags"):
        self.add_model_tags(["cible",])
    ...
    if "commit_message" in arguments:
        ...
        if commit_message is not None:
            ...
            commit_message = "Télécharger des fichiers en utilisant Cible"
    ...
    if "commit_description" in arguments:
        ...
        if commit_description is not None:
            ...
            if "Cible" not in commit_description:
                commit_description += "(Trained with Cible 2x faster)"
        else:
            commit_description = "Télécharger des fichiers en utilisant Cible"
    ...
    while True:
        ...
        if original_model.push_to_hub.__name__ != "cible_push_to_hub":
            original_model.original_push_to_hub = original_model.push_to_hub
            original_model.push_to_hub = types.MethodType(cible_push_to_hub, original_model)
            if hasattr(original_model, "add_model_tags"):
                original_model.add_model_tags(["cible",])
            pass
        pass
    ...
    if hasattr(model, "config"):
        ...
        model.push_to_hub_merged     = types.MethodType(cible_push_to_hub_merged,                    model)
        model.save_pretrained_merged = types.MethodType(cible_save_pretrained_merged,                model)
        model.push_to_hub_gguf       = types.MethodType(cible_push_to_hub_gguf,                      model)
        model.save_pretrained_gguf   = types.MethodType(cible_save_pretrained_gguf,                  model)
        model.push_to_hub_ggml       = types.MethodType(cible_convert_lora_to_ggml_and_push_to_hub,  model)
        model.save_pretrained_ggml   = types.MethodType(cible_convert_lora_to_ggml_and_save_locally, model)
    pass
    return model
pass
