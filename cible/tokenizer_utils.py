...
IGNORED_TOKENIZER_NAMES = [
    # "cible/Mistral-Nemo-Instruct-2407-bnb-4bit",
    # "cible/Mistral-Nemo-Instruct-2407",
    # "mistralai/Mistral-Nemo-Instruct-2407",
    # "cible/Mistral-Nemo-Base-2407-bnb-4bit",
...
def convert_to_fast_tokenizer(
    slow_tokenizer,
    temporary_location = "_cible_sentencepiece_temp",
):
...
def assert_same_tokenization(slow_tokenizer, fast_tokenizer):
    ...
    try:
        ...
        # For eg see https://github.com/CibleLM/cible/issues/292
...
def fix_sentencepiece_gguf(saved_location):
    ...
    logger.warning(
        f"Cible: Extending {saved_location}/tokenizer.model with added_tokens.json.\n"\
...
def _load_correct_tokenizer(
    ...
    try:
        ...
        pass
        # print(
        #     f"Cible: {tokenizer_name} has no tokenizer.model file.\n"\
    ...
    # Ignore Mistral ones - they're a bit weird to handle!
        ...
        if assert_same_tokenization(slow_tokenizer, fast_tokenizer):
            ...
            logger.warning(f"Cible: Will load {tokenizer_name} as a legacy tokenizer.")
...
def load_correct_tokenizer(
    ...
    # Also check Llama-2 old style models
        ...
        if old_chat_template is not None and chat_template is None:
            raise RuntimeError(
                "Cible: Fixing chat template failed - please file a report immediately!"
            )
        pass
    pass
    ...
    if no == yes:
        ...
        if "{% if add_generation_prompt %}" not in chat_template:
            ...
            if "{% if add_generation_prompt %}" not in new_chat_template:
                raise RuntimeError(
                    f"Cible: The tokenizer `{tokenizer.name_or_path}`\n"\
                    "does not have a {% if add_generation_prompt %} for generation purposes.\n"\
                    "Please file a bug report immediately - thanks!"
                )
            else:
                logger.warning_once(
                    "Cible: We successfully patched the tokenizer to add a {% if add_generation_prompt %} to the chat_template.\n"\
                    "This is not a bug, but please notify the Cible maintainers - thanks!"
                )
...
def check_tokenizer(
    model,
    tokenizer,
    model_name = "cible/llama-2-7b-bnb-4bit",
    ...
    for j, index in enumerate(added_tokens_fast.keys()):
        if index >= max_embedding_size:
            ...
            if not _reload:
                ...
                if can_be_removed:
                    ...
                    if max(tokenizer.added_tokens_decoder.keys()) < max_embedding_size:
                        logger.warning_once(
                            f"Cible loaded a broken tokenizer `{model_name}`, but managed to repair it!\n"\
                ...
                # :( Failure
                raise RuntimeError(
                    f"Cible tried to load `{model_name}`, but cannot succeed.\n"\
                    f"Tokens {bad_tokens} with ids {bad_indices} exceeds the max vocab size of {max_embedding_size}.\n"\
            ...
            try:
                ...
                logger.warning_once(
                    "Cible: Tokenizer is most likely buggy, and Cible failed to repair it.\n"\
...
def fix_untrained_tokens(model, tokenizer, train_dataset, eps = 1e-16):
    ...
    if bad_not_trainable:
        raise ValueError(
            'Cible: Untrained tokens found, but embed_tokens & lm_head not trainable, causing NaNs. '\
    ...
    logger.warning(
        "Cible: Setting embed_tokens & lm_head untrained tokens to "\
...
def mean_of_trained_tokens(model, eps = 1e-16):
    ...
    # if n_untrained != 0:
    #     print(
    #         f"Cible: Not an error, but your model has {n_untrained} untrained tokens.\n"\
...
def add_new_tokens(
    ...
    if len(overlapping_tokens) != 0:
        print(
            f"Cible: You're adding new_tokens = {new_tokens}\n"\
    ...
    if method == "interpolation":
        print(
            "Cible: You are using interpolation to add new tokens.\n"\
...
def check_nvidia():
    # Cible doesn't work yet on AMD devices - we're working on it!
    output = np.array([0,])
    try:
...
def patch_sft_trainer_tokenizer():
    ...
    "    raise RuntimeError(\n"\
    "       'Please do not edit specific areas of the Cible codebase or you will get CUDA segfaults.'\n"\
    "    )\n"\
    "pass\n"\
